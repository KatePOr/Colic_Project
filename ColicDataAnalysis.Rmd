...
```
```{r}
# One row with NA values in Surgery is removed from colic_training and colic_test
...
colic_test[, c(1,2,7,8,9,10,11,12,13,14,15,18,19,20)] <- lapply(colic_test[, c(1,2,7,8,9,10,11,12,13,14,15,18,19,20)], as.character)
colic_training[, c(1,2,7,8,9,10,11,12,13,14,15,18,19,20)] <- lapply(colic_training[, c(1,2,7,8,9,10,11,12,13,14,15,18,19,20)], as.character)
...
```
```{r}
# Replace NA values with mode for extremity_temperature, peripheral_pulse, mucous membrane, capillary_refill_time, pain_rating, peristalsis, abdominal_distension

```

```{r}

#Replace NA values with mode of specific sections for rectal_exam_feces, abdomen, abdominocentesis_appearance, abdominocentesis_total_protein
tallied <- colic_training %>% group_by(rectal_exam_feces, abdominal_distension, mucous_membrane, capillary_refill_time, peristalsis) %>% 
  summarize(frequency = n()) %>% 
  filter(frequency == max(frequency)) %>%
  ungroup()
colic_training$rectal_exam_feces <- replace(colic_training$rectal_exam_feces, is.na(colic_training$rectal_exam_feces), tallied$rectal_exam_feces[tallied$abdominal_distension==colic_training$abdominal_distension & tallied$mucous_membrane==colic_training$mucous_membrane & tallied$capillary_refill_time==colic_training$capillary_refill_time & tallied$peristalsis==colic_training$peristalsis] )
